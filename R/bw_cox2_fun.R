#' coxph model with backward elimination using p value with threshold 0.05
#'
#'
...
bw_cox2_fun=function(r,data,cvK,fitform_ogl,formula1,formula2,formula3, formula4, timess){
  ...
  for (j in 1:cvK) {
    print(j)
    test_id = cvTools::cvSets$subsets[cvSets$which == j]
    test = data[test_id, ]
    train = data[-test_id, ]

    original_cox1=rms::cph(fitform_ogl,x=TRUE,y=TRUE,data=train)
    z<-rms::fastbw(original_cox1, rule="p",sls=.05)
